...
- ``SourceAC`` (ideal AC source), driven through ``.run_with_AC_source()``;
...
def test_no_load_stator_current_rms(results_no_load):
    ...
    # Document: ~2.85 A rms; tolerance set to ±5 % of expected value
    assert_allclose(ia_rms, 2.85, rtol=0.05, atol=0.1,
...
def test_no_load_stator_current_peak(results_no_load):
    ...
    # Document: ~4.04 A peak; tolerance ±5%
    assert_allclose(ia_peak, 4.04, rtol=0.05, atol=0.1,
...
def test_no_load_speed(results_no_load):
    ...
    assert_allclose(speed_rpm, 1799.0, atol=5.0,
...
def test_nominal_load_stator_current_rms(results_nominal_load):
    ...
    # Document: ~4.25 A rms; tolerance ±5 %
    assert_allclose(ia_rms, 4.38, rtol=0.05, atol=0.15,
...
def test_nominal_load_stator_current_peak(results_nominal_load):
    ...
    # Document: ~6.0 A peak; tolerance ±5 %
    assert_allclose(ia_peak, 6.19, rtol=0.05, atol=0.2,
...
def test_nominal_load_speed(results_nominal_load):
    ...
    # Document: 1705 RPM; tolerance ±5 RPM
    assert_allclose(speed_rpm, 1705.0, atol=5.0,
...
def test_starting_current_rms(results_locked_rotor):
    ...
    # Document: ~25.64 A rms; tolerance ±5 %
    assert_allclose(ia_rms, 25.64, rtol=0.5, atol=0.5,
...
def test_starting_current_peak(results_locked_rotor):
    ...
    # Document: ~36.26 A peak; tolerance ±5 %
    assert_allclose(ia_peak, 36.26, rtol=0.05, atol=0.5,
...
def test_starting_electromagnetic_torque(results_locked_rotor):
    ...
    # Document: ~17.71 N.m; tolerance ±5 %
    assert_allclose(te_mean, 17.71, rtol=0.05, atol=0.3,
...
def results_inverter_vf_nominal_no_load(motor):
    ...
    tf = 3.0
    t = np.arange(0, tf + dt, dt)
    return motor.run_with_inverter(
        t,
        time_step=1e-5,
        load_torque_entrance_time=tf + 1.0,  # load applied after simulation ends
        load_torque_ratio=0.0,
